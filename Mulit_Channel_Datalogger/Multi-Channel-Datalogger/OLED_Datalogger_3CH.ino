...
void runLogger()
{
  ...
  if ((getMin == 00) || (getMin == 0) || (getMin == 30))
  {
    actLogger();
    delay(59000);
  }
...
void actLogger()
{
  ...
  if (screenCycle != 4) //If dispaly screen is blank dont push title
  {
    operationStatus = 0;
    setTitle();
  }
...
void makeDataString()
{
  ...
  //Write Day File
  String fileDayString = "datalogger/day/";
  fileDayString += getDay;
  fileDayString += "-";
  fileDayString += getMonth;
  fileDayString += "-";
  fileDayString += getYear;
  fileDayString += ".csv";
  char fileDayChar[fileDayString.length() + 1];
  fileDayString.toCharArray(fileDayChar, fileDayString.length() + 1);

  LFile dataFile = Drv.open(fileDayChar, FILE_WRITE); //Open File
  if (dataFile)
  {
  ...
  //Write Month File
  String fileMonthString = "datalogger/month/";
  fileMonthString += getMonth;
  fileMonthString += "-";
  fileMonthString += getYear;
  fileMonthString += ".csv";
  char fileMonthChar[fileMonthString.length() + 1];
  fileMonthString.toCharArray(fileMonthChar, fileMonthString.length() + 1);
  dataFile = Drv.open(fileMonthChar, FILE_WRITE); //Open File
  if (dataFile)
  {
    dataFile.println(datalogWrite);
    dataFile.close();
  ...
  //Write Year File
  String fileYearString = "datalogger/year/";
  fileYearString += getYear;
  fileYearString += ".csv";
  char fileYearChar[fileYearString.length() + 1];
  fileYearString.toCharArray(fileYearChar, fileYearString.length() + 1);
  dataFile = Drv.open(fileYearChar, FILE_WRITE); //Open File
  if (dataFile)
  {
    dataFile.println(datalogWrite);
    dataFile.close();
